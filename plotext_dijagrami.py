...
def jednostavi_barcahrt():
    plt.simple_bar(zupanije, stan_2021, title='Broj stanovnika po županijama 2021-jednostavni barchart', width=150)
    plt.show()

def grupirani_barchart():
    plt.multiple_bar(zupanije4, [stan2021, stan2011, stan2001], labels=["popis 2021", "popis 2011", "popis 2001"], width=3 / 5)
    plt.title("Broj stanovništva po županijama")
    plt.show()

def jednostavni_grupirani():
    plt.simple_multiple_bar(zupanije4, [stan2021, stan2011, stan2001], labels=["popis 2021", "popis 2011", "popis 2001"], title="Broj stanovništva po županijama", width=120)
    plt.show()
...
def jednostavni_slozeni():
    plt.simple_stacked_bar(godine, [muskarci, zene], labels=["muskarci", "zene"], title="Omjer žena i muškaraca po popisima", width=100)
    plt.show()

def histogram_vertikalni():
    plt.hist(godine2021, bins=10, color="blue", width=3/5)
...
def jednostavni_timeline():
    plt.simple_bar(dan_predmet, trajanje, width=100, title='Raspored sati od ponedjeljka do četvrtka', color="cyan")
    plt.show()

def vertikalni_timeline():
    plt.bar(dan_predmet, trajanje, width=0.1, color="cyan")
